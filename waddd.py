...
"""
Created on Wed 21 may 21:29:56 2022
...
def random_word_generator(url=r"https://raw.githubusercontent.com/charlesreid1/five-letter-words/master/sgb-words.txt"):
...
def wordle_game(provided_text="", input_text=""):
    global iterations
    if iterations == 0:
        iterations += 1
    if provided_text == input_text:
        ...
        for i in range(0, len(provided_text)):
            if provided_text[i] in input_text:
                ...
                if index == i:
                    print(provided_text[i], " is at right place", i + 1)
                    print("")
                else:
                    print(provided_text[i], " is exist but at random place")
                    print("")
        return False


def INP_check(provided_text):
    if len(provided_text) > 5 or len(provided_text) < 5:
        print("word should be of length 5, please try again")
        return False
    elif not re.match("^[a-z]*$", provided_text):
        print("Only a to z letters are allowed")
        return False
    else:
        return True


def wordle_wrapp(text_of_the_day="Hello"):
    print("**************************=============**************************")
    print("")
    print("this is PAYAL's Wordle Game!")
    print("")
    print("**************************=============**************************")
    print("")
    ...
    for i in range(6):
        ...
        while (True):
            ...
            res = INP_check(provided_text)
            if res:
                break
        print("")
        if wordle_game(provided_text, input_text):
            flag = True
            return flag

    if not flag:
        print("you loose! Max Attempts reached, play again!")


def calling_wrapp(text_of_the_day="Hello"):
    while (True):
        flag = wordle_wrapp(text_of_the_day)
        if flag:
            break
...
if __name__ == "__main__":
    ...
    text_of_the_day = random_word_generator(url)
    calling_wrapp(text_of_the_day)
